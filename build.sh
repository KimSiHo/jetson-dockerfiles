...
BUILD_OPTS_WITH_SPECIFIC_TAG="-t $DOCKER_IMAGE_URL \
  $BUILD_OPTS"
BUILD_OPTS_WITH_LATEST_TAG="-t $DOCKER_LATEST_IMAGE_URL \
  $BUILD_OPTS"
BUILD_ARGS="$BUILD_ARGS $DOCKER_FILE_HOME"

docker run --rm --privileged tonistiigi/binfmt --install all

echo "
=== build command start ===
docker buildx build --platform linux/arm64 $BUILD_OPTS_WITH_SPECIFIC_TAG $BUILD_ARGS
=== build command end ===
"
docker buildx build --platform linux/arm64 --load $BUILD_OPTS_WITH_SPECIFIC_TAG $BUILD_ARGS

if [ ! -z "${DOCKER_LATEST_IMAGE_URL}" ] ; then
  echo "
=== latest tag build command start ===
docker buildx build --platform linux/arm64 $BUILD_OPTS_WITH_LATEST_TAG $BUILD_ARGS
=== latest tag build command end ===
"
  docker buildx build --platform linux/arm64 --load $BUILD_OPTS_WITH_LATEST_TAG $BUILD_ARGS
fi
