...
echo "
=== build command start ===
docker buildx build --platform linux/arm64 --load --progress=plain $BUILD_OPTS_WITH_SPECIFIC_TAG $BUILD_ARGS -f $DOCKERFILE $DOCKER_FILE_HOME
=== build command end ===
"
...
docker buildx build --platform linux/arm64 --load --progress=plain $BUILD_OPTS_WITH_LATEST_TAG $BUILD_ARGS -f $DOCKERFILE $DOCKER_FILE_HOME
